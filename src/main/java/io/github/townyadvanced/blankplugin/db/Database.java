package io.github.townyadvanced.blankplugin.db;
...
public class Database {
	...
	final static String rootFolderPath = BlankPlugin.getPlugin().getDataFolder().getPath();
	protected final static Queue<Runnable> queryQueue = new ConcurrentLinkedQueue<>();
	private static BukkitTask task; 
	
	public Database(BlankPlugin plugin) {
		if (!FileMgmt.checkOrCreateFolders(
			...
			BlankPlugin.severe("Could not create default folders");
	...
	public static boolean loadFileList(FileType type, Consumer<UUID> consumer) {
		BlankPlugin.info("Searching for " + type.foldername + "...");
		...
		if (files.length == 0) {
			BlankPlugin.info("No " + type.foldername + " found.");
			return true;
		}
		
		BlankPlugin.info("Loading " + type.foldername + " list...");
